...
function CalendarInit()				///Create panel
{
	...
	htmlAll="<div id='"+this.sDIVID+"' style='display:none;position:absolute;width:"+this.Width123+";border:"+this.divBorderCss+";padding:2px;background-color:#FFFFFF'>";
	htmlAll+="<div align='center'>";
	/// Month
	htmloMonth="<select id='"+this.sMONTHID+"' onchange=CalendarMonthChange("+this.instanceName+") style='width:100'>";
	for(i=0;i<12;i++)
	{			
	...
	htmloMonth+="</select>";
	/// Year
	htmloYear="<select id='"+this.sYEARID+"' onchange=CalendarYearChange("+this.instanceName+") style='width:100'>";
	...
	for(i=0;i<=6;i++)
	{
		...
		for(j=0;j<7;j++)
		{
			...
			if(i==0)
			{
				htmloDayTable+="<td height='10' align='center' valign='middle' style='cursor:hand'>";
				htmloDayTable+=this.weekDaySting[j]+"</td>"
			}
			else
			{
				htmloDayTable+="<td height='20px' align='center' valign='middle' style='cursor:hand'";
